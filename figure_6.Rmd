...
path_test <- file.path(tempdir(), "p_test.pdf")
cowplot::ggsave2(
  path_test, 
  p_out, 
  height = 29.4 * 0.9, 
...
var_exp_to_shape <- c(
  "bcg_diff" = "triangle filled",
  "day_365_diff" = "circle filled",
...
plot_tbl_legend_diff <- purrr::map_df(rev(unique(names(var_exp_to_x))), function(var_exp) {
  tibble::tibble(
    ...
    shape = var_exp_to_shape[var_exp],
    fill = var_exp_to_fill[var_exp],
...
p_legend <- ggplot() + 
  ...
  geom_point(
    ...
    aes(x = x - 0.0175, y = y, fill = var_exp, 
        shape = var_exp), 
    size = 3
  ) +
  geom_text(
...
path_test_full <- file.path(tempdir(), "p_test_full.pdf")
cowplot::ggsave2(
  path_test_full, 
  p_test_full, 
  height = 29.4, 
